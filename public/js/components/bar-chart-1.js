...
(function () {
  document.addEventListener('DOMContentLoaded', function () {
    console.log("bar-chart-1 active"); // Let's get this party started.
    ...
    button.addEventListener("click", function () {
      ...
      const svg = d3.select("#svgOutput").append("svg");
      svg.attr("viewBox", [0, 0, w, h]).attr("preserveAspectRatio", "xMidYMid meet");
      const yScale = d3.scaleLinear().domain([0, d3.max(resultsArray, d => d.count)]).range([25, h - padding]);
      svg.selectAll("rect.bar").data(resultsArray).enter().append('rect').attr('class', 'bar').attr('x', (d, i) => {
      ...
      }).attr("data-count", d => d.count).on("click", barClickHandler);
      svg.selectAll("text.title").data(resultsArray).enter().append("text").attr('class', 'title').text(d => d.char).attr('x', (d, i) => {
        return i * (w / resultsArray.length) + 10;
      }).attr("y", d => {
        return h - 10;
      }).style('font-family', 'Courier New').style('font-size', '0.75rem').style('font-weight', 'bold').style('fill', 'white');
      svg.selectAll("text.count").data(resultsArray).enter().append("text").attr('class', 'count').text(d => d.count).attr('x', (d, i) => {
        return i * (w / resultsArray.length) + 20;
      }).attr("y", d => {
        return h - yScale(d.count) - 10;
      }).style('font-size', '0.75rem').style('font-weight', 'bold').style('font-family', 'Courier New').style('fill', 'navy').attr('transform', (d, i) => {
      ...
      function barClickHandler(d, i) {
        ...
        svg.append('text').attr("x", "25").attr("y", "150").attr('class', 'bigLabel').style('font-family', 'Georgia, serif').style('font-size', '10rem').style('font-weight', 'bold').style('fill', 'navy').text(d.char + " ∙ " + d.count);
      }
    });
  });
})();
